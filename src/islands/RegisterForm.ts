...
export default function RegisterForm(): NixTemplate {
  const name = signal("");
  const email = signal("");
  const action = nixJsAction("register", { page: "/register" });
  ...
  return html`
    <form class="card" style="max-width: 420px;" novalidate @submit=${submit} action="/__nix-js/actions" method="POST">
      <input type="hidden" name="__nix_js_action_name" value="register" />
      <input type="hidden" name="__nix_js_action_page" value="/register" />
